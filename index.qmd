...
### New — Running 8× H100 on-prem for clinical AI *(July 2026)*

Eight NVIDIA H100s inside a hospital firewall, running open LLMs on clinical
data that legally cannot leave the building. The hardware is the easy part —
the real engineering is a **three-location PHI-residency design** (cluster =
notes, jump box = structured PHI, laptop = none) and serving through an
OpenAI-compatible vLLM endpoint so downstream tools never touch the platform
underneath.

[Read the full post →](advice/onprem-h100-openshift-clinical-ai.qmd) ·
...
- [Running 8× H100 on-prem for clinical AI](advice/onprem-h100-openshift-clinical-ai.qmd) —
  on-prem GPUs + OpenShift AI + a three-location PHI-residency design.
